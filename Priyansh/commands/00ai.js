...
module.exports.handleEvent = async function({ api, event }) {
  ...
  global.babuSessions = global.babuSessions || {};

  // STEP 1: Trigger "babu"
  if (body && body.trim().toLowerCase() === "babu") {
    global.babuSessions[threadID] = true;
    return api.sendMessage("Jaldi Bol Time Nahi Hai🤨", threadID, messageID);
  }

  // STEP 2: Only active session
  const isActive = global.babuSessions[threadID];
  const isReplyTobabu = messageReply && messageReply.senderID == api.getCurrentUserID();
  if (!isActive || !isReplyTobabu) return;

  // Chat history
  global.babu = global.babu || {};
  global.babu.chatHistory = global.babu.chatHistory || {};
  const chatHistory = global.babu.chatHistory;
  ...
  // Prompt
  const prompt = `
Tum ek naughty, romantic, confident ladka ho — jiska naam babu hai.
...
- Koi tumhe babu kahe to funny insult kar dena
...
`;
  ...
  try {
    ...
    const babuReply = (typeof res.data === "string" ? res.data : JSON.stringify(res.data)).trim();

    chatHistory[senderID].push(`babu: ${babuReply}`);
    return api.sendMessage(babuReply, threadID, messageID);
  } catch (err) {
...
module.exports.run = async function({ api, event }) {
  return api.sendMessage("Mujhse baat karne ke liye pehle 'babu' likho, phir mere message ka reply karo 😎", event.threadID, event.messageID);
};
